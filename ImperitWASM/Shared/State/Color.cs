...
namespace ImperitWASM.Shared.State
{
	...
	{
		private static string ToHex(byte num) => num.ToString("x2", CultureInfo.InvariantCulture);
		private static byte FromHex(string s) => byte.Parse(s, NumberStyles.HexNumber, CultureInfo.InvariantCulture);
		...
		public static Color Parse(string str) => new Color(FromHex(str[1..3]), FromHex(str[3..5]), FromHex(str[5..7]), str.Length < 9 ? 255 : FromHex(str[7..9]));
		private static byte Mix(byte a, byte b, int w1, int w2) => (byte)(((a * w1) + (b * w2)) / (w1 + w2));
		private static byte Supl(byte x, byte y) => (byte)(255 - ((255 - x) * (255 - y) / 255));
		...
		public Color(double H, double S, double V)
		{
			...
			static byte clamp(int i) => i < 0 ? 0 : i > 255 ? 255 : (byte)i;
			r = clamp((int)(R * 255.0));
			g = clamp((int)(G * 255.0));
			b = clamp((int)(B * 255.0));
			a = 255;
