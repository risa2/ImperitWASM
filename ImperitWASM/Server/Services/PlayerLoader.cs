...
namespace ImperitWASM.Server.Services
{
	...
	public class PlayerLoader : IPlayerLoader
	{
		...
		void Add(int gameId, int order, Player player, bool fromTransaction) => db.Transaction(!fromTransaction, () =>
		{
			db.Command(@"INSERT INTO Player (Name,Active,Alive,GameId,Human,Money,""Order"",Password) VALUES (@x0,@x1,@x2,@x3,@x4,@x5,@x6,@x7)",
			...
			foreach (var action in player.Actions)
			{
				...
				{
					db.Command("INSERT INTO Action (GameId,Player,Province,Type) VALUES (@x0,@x1,@x2,@x3)", gameId, player.Order, manoeuvre.Province, "Manoeuvre");
				...
				{
					db.Command("INSERT INTO Action (GameId,Player,Debt,Type) VALUES (@x0,@x1,@x2,@x3)", gameId, player.Order, loan.Debt, "Loan");
				}
			}
		});
		...
		{
			db.Command("DELETE FROM Player WHERE GameId=@x0", gameId);
			foreach (var (i, player) in players.Select((p, i) => (i, p)))
			{
				Add(gameId, i, player, true);
			}
		});
		public bool IsNameFree(string name) => !name.Any(char.IsDigit) && db.Query<int>("SELECT Count(Name) FROM Player WHERE Name=@x0", name).Single() == 0;
