...
namespace ImperitWASM.Shared.Cvt
{
	public class SettingsConverter : JsonConverter<Settings>
	{
		public override Settings Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
		{
			...
			var types = JsonSerializer.Deserialize<ImmutableArray<SoldierType>>(root.GetProperty("SoldierTypes").GetRawText());
			return new Settings(new Ratio(root.GetProperty("DefaultInstability").GetInt32()), root.GetProperty("Interest").GetDouble(), root.GetProperty("DebtLimit").GetInt32(), root.GetProperty("DefaultMoney").GetInt32(), root.GetProperty("MountainsWidth").GetInt32(), Color.Parse(root.GetProperty("LandColor").GetString().Must()), Color.Parse(root.GetProperty("MountainsColor").GetString().Must()), Color.Parse(root.GetProperty("SeaColor").GetString().Must()), types, types.Lookup(), root.GetProperty("FinalLandsCount").GetInt32(), TimeSpan.FromSeconds(root.GetProperty("CountdownTime").GetInt32()), JsonSerializer.Deserialize<Graph>(root.GetProperty("Graph").GetRawText()).Must(), JsonSerializer.Deserialize<ImmutableArray<ProvinceData>>(root.GetProperty("ProvinceData").GetRawText()));
		}
