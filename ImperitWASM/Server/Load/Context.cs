...
namespace ImperitWASM.Server.Load
{
	public class Context : DbContext
	{
		...
		protected override void OnConfiguring(DbContextOptionsBuilder opt) => opt.UseSqlite("Data Source=" + System.IO.Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory ?? ".", "Files/imperit.db"));
		protected override void OnModelCreating(ModelBuilder mod)
		{
			...
			_ = mod.OneToMany<EntityPlayer, EntityProvinceAction>(x => x.EntityPlayer!).IsRequired();
			_ = mod.OneToMany<EntityProvince, EntityProvinceAction>(x => x.EntityProvince!, x => x.EntityProvinceActions).IsRequired();
			_ = mod.OneToMany<EntityProvinceAction, EntitySoldier>(x => x.EntityProvinceAction!, x => x.EntitySoldiers).IsRequired();
		}
	}
}
