...
namespace ImperitWASM.Shared.Func
{
	public static class Extensions
	{
		...
		public static void InsertMatch<T, K>(this IList<T> s1, IEnumerable<T> s2, Func<T, K> key, Func<T, T, T> match) where K : IEquatable<K>
		{
			foreach (var t2 in s2)
			{
				var index = s1.Find(t1 => key(t1).Equals(key(t2)));
				if (index is int i)
				{
					s1[i] = match(s1[i], t2);
				}
