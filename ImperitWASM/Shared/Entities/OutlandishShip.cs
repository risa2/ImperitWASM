...
namespace ImperitWASM.Shared.Entities
{
	public class OutlandishShip : Ship
	{
		...
		public OutlandishShip(string name, string text, string symbol, int attackPower, int defensePower, int weight, int price, int capacity, int speed)
			: base(name, text, symbol, attackPower, defensePower, weight, price, capacity) => Speed = speed;
		...
		public override bool IsRecruitable(Province province) => province is Land l && l.HasExtra(this);
	}
}
