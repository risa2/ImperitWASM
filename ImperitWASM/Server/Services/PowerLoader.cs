...
namespace ImperitWASM.Server.Services
{
	...
	public class PowerLoader : IPowersLoader
	{
		...
		public void Add(int gameId, IEnumerable<Power> powers, bool fromTransaction)
		{
		...
		public ImmutableArray<Powers> Get(int gameId)
		{
			...
			return db.Query<int, bool, int, int, int, int>(@"SELECT ""Order"", Alive, Final, Income, Money, Soldiers FROM Power WHERE GameId=@x0 ORDER BY ""Order"" ").GroupBy(p => p.Item1).Select(pg => new Powers(pg.Select(p => new Power(p.Item2, p.Item3, p.Item4, p.Item5, p.Item6)).ToImmutableArray())).ToImmutableArray();
		}
	}
}
