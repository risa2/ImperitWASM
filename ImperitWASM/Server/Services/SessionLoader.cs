...
namespace ImperitWASM.Server.Services
{
	...
	public class SessionLoader : ISessionLoader
	{
		...
		public string Add(int player, int gameId, bool fromTransaction) => db.Transaction(!fromTransaction, () =>
		{
			...
			while (db.Query<int>("SELECT COUNT(*) FROM Session WHERE Key = @0", key).First() > 0)
			{
		...
		public bool IsValid(int player, int gameId, string key) => db.Query<int>("SELECT COUNT(*) FROM Session WHERE Key=@0 AND GameId=@1 AND Player=@2", key, gameId, player).First() > 0;
